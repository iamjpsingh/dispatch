...
import { RecordEventSchema, SeedSchema, CustomReportSchema } from '@dispatch/shared/analytics'

// ============================================================================
// Report Builder column/filter whitelists (referenced by the handlers below)
// ============================================================================
...
const analyticsRoutes = new Hono()
  ...
  .get('/analytics/summary', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const summary = await analyticsService.getSummary(orgId)
    return success(c, summary)
  })
  // Get campaign report
  .get('/analytics/campaigns/:campaignId', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
  ...
  .get('/analytics/campaigns', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
  ...
  .get('/analytics/campaigns/:campaignId/links', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.param('campaignId')
  ...
  .get('/analytics/devices', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
  ...
  .get('/analytics/geo', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.query('campaign_id')
  ...
  .get('/analytics/time', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
  ...
  .get('/analytics/export/campaign/:campaignId', requirePermission(PERMISSIONS.ANALYTICS_EXPORT), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.param('campaignId')
  ...
  // Export summary
  .get('/analytics/export/summary', requirePermission(PERMISSIONS.ANALYTICS_EXPORT), async (c) => {
    const orgId = getOrgId(c)
    const format = (c.req.query('format') || 'json') as 'csv' | 'json'
  ...
  .post('/analytics/events', requirePermission(PERMISSIONS.ANALYTICS_VIEW), zValidator('json', RecordEventSchema), async (c) => {
    const orgId = getOrgId(c)
  ...
  .post('/analytics/seed', requirePermission(PERMISSIONS.ANALYTICS_VIEW), zValidator('json', SeedSchema), async (c) => {
    const orgId = getOrgId(c)
    const { campaignId, campaignName, stats } = c.req.valid('json')
  ...
  // ==========================================================================
  .get('/analytics/email-health', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const summary = await analyticsService.getSummary(orgId)
  ...
  // ==========================================================================
  .post('/analytics/custom-report', requirePermission(PERMISSIONS.ANALYTICS_VIEW), zValidator('json', CustomReportSchema), async (c) => {
    const orgId = getOrgId(c)
    const body = c.req.valid('json')
  ...
  .get('/analytics/report-builder/schema', requirePermission(PERMISSIONS.ANALYTICS_VIEW), (c) => {
    return success(c, {
  ...
  .get('/analytics/campaigns/:id/geo', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
  ...
  .get('/analytics/campaigns/:id/devices', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.param('id')
  ...
  .get('/analytics/campaigns/:id/clients', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.param('id')
  ...
  .get('/analytics/campaigns/:id/referrers', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
    const campaignId = c.req.param('id')
  ...
  .get('/analytics/contacts/:contactId/profile', requirePermission(PERMISSIONS.ANALYTICS_VIEW), async (c) => {
    const orgId = getOrgId(c)
