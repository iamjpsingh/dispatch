...
export function requirePermission(...permissions: Permission[]) {
  return async (c: Context, next: Next) => {
    ...
    if (!orgId) {
      if (await rbacService.isPlatformAdmin(user.id)) {
        return next()
      }
    ...
    const hasAccess = permissions.length === 1
      ? await rbacService.hasPermission(user.id, orgId, permissions[0])
      : await rbacService.hasAllPermissions(user.id, orgId, permissions)
...
export function requireAnyPermission(...permissions: Permission[]) {
  return async (c: Context, next: Next) => {
    ...
    if (!(await rbacService.hasAnyPermission(user.id, orgId, permissions))) {
      return c.json({
        success: false,
        message: 'Insufficient permissions',
        required: permissions,
...
export function requireOrgMember() {
  return async (c: Context, next: Next) => {
    ...
    const isAdmin = await rbacService.isPlatformAdmin(user.id)
    const isMember = await rbacService.isMember(user.id, orgId)
    if (!isAdmin && !isMember) {
